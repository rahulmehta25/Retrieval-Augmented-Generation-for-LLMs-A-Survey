# Kubernetes Add-ons for RAG System
...
# EBS CSI Driver (if not using EKS add-on)
resource "helm_release" "aws_ebs_csi_driver" {
  count = var.ebs_csi_driver_version != "" ? 0 : 1  # Only deploy if not using EKS add-on
...
resource "aws_iam_role" "ebs_csi_driver" {
  count = var.ebs_csi_driver_version != "" ? 0 : 1
